...
{
	this->_spellBook.learnSpell(spell_to_learn);
}

void Warlock::launchSpell(const std::string spell_name, const ATarget & target)
{

	if (this->_spellBook.getSpells().count(spell_name) == 1)
	{
		this->_spellBook.getSpells()[spell_name]->launch(target);
...
{
	this->_spellBook.forgetSpell(spell_to_forget);
